...
class _ActiveProductsScreenState extends State<ActiveProductsScreen>
  ...
  ActiveProductsBloc activeProductsBloc;
  ...
  void initState() {
    ...
    productsList = List();
    activeProductsBloc = BlocProvider.of<ActiveProductsBloc>(context);

    activeProductsBloc.add(GetActiveProductsEvent());

    activeProductsBloc.listen((state) {
      print('Active Products STATE :: $state');
    });
  }
    ...
    return Scaffold(
      body: Column(
        children: <Widget>[
          ...
          Expanded(
            child: BlocBuilder(
              bloc: activeProductsBloc,
              buildWhen: (previous, current) {
                if (current is GetActiveProductsCompletedState ||
                    current is GetActiveProductsInProgressState ||
                    current is GetActiveProductsFailedState) {
                  return true;
                }
                return false;
              },
              builder: (context, state) {
                if (state is GetActiveProductsInProgressState) {
                ...
                if (state is GetActiveProductsFailedState) {
                  return Column(
                    crossAxisAlignment: CrossAxisAlignment.center,
                ...
                if (state is GetActiveProductsCompletedState) {
