...
class _InactiveProductsScreenState extends State<InactiveProductsScreen>
  ...
  InactiveProductsBloc inactiveProductsBloc;
  ...
  void initState() {
    ...
    productsList = List();
    inactiveProductsBloc = BlocProvider.of<InactiveProductsBloc>(context);

    inactiveProductsBloc.add(GetInactiveProductsEvent());

    inactiveProductsBloc.listen((state) {
      print('Inactive Products STATE :: $state');
    });
  }
    ...
    return Scaffold(
      body: Column(
        children: <Widget>[
          ...
          Expanded(
            child: BlocBuilder(
              bloc: inactiveProductsBloc,
              buildWhen: (previous, current) {
                if (current is GetInactiveProductsCompletedState ||
                    current is GetInactiveProductsInProgressState ||
                    current is GetInactiveProductsFailedState) {
                  return true;
                }
                return false;
              },
              builder: (context, state) {
                if (state is GetInactiveProductsInProgressState) {
                ...
                if (state is GetInactiveProductsFailedState) {
                  return Column(
                    crossAxisAlignment: CrossAxisAlignment.center,
                ...
                if (state is GetInactiveProductsCompletedState) {
